...
class PropCheckService:
    ...
    def resolve_property(self, elem, prop_path: str, case_sensitive: bool) -> Optional[str]:
        ...
        if not case_sensitive:
            ...
            return self.normalize_value(props.get(prop_key)) if prop_key else None
        return self.normalize_value(psets.get(pset_name, {}).get(prop_name))

    def matches_entity_filters(self, elem, filters: List[Dict[str, List[str]]], case_sensitive: bool) -> bool:
        if not filters:
            return True
        for filter_group in filters:
            match = True
            for key, allowed_vals in filter_group.items():
                actual_val = self.resolve_property(elem, key, case_sensitive)
    ...
    def check_value(self, val: Optional[str], chk: Dict[str, Any], missing_policy: str) -> Optional[str]:
            ...
            for rule in rules.get("rules", []):
                ...
                for elem in track(elems, description=f"{Path(fpath).name} • {rid}"):
                    if elem.is_a().lower() in ignored_entities:
                        continue
                    if not self.matches_entity_filters(elem, filters, case_sensitive):
                        continue
                    total_objects += 1
                    for chk in checks:
                        prop = chk["property"]
                        actual_val = self.resolve_property(elem, prop, case_sensitive)
                        severity = self.check_value(actual_val, chk, missing_policy)
